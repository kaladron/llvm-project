...
//
// Like glibc and musl, a single static buffer and struct passwd are reused
// across getpwent, getpwnam, and getpwuid per POSIX ("The return value may
// point to a static area which is overwritten by a subsequent call to
// getpwent(), getpwnam(), or getpwuid()"), growing only to the high-water mark
// of the largest record seen. endpwent() closes the file stream without freeing
// the buffer so that pointers returned prior to endpwent() remain valid until
