...
Expr *ExprPool::intern(const Expr &e) {
...
Expr *ExprPool::empty_set() { return intern(Expr(ExprKind::EmptySet)); }
Expr *ExprPool::empty_str() { return intern(Expr(ExprKind::EmptyStr)); }
Expr *ExprPool::make_lit(char c) { return intern(Expr(c)); }

Expr *ExprPool::make_concat(Expr *l, Expr *r) {
  if (!l || !r)
    return nullptr;
...
Expr *ExprPool::make_alt(Expr *l, Expr *r) {
  if (!l || !r)
    return nullptr;
