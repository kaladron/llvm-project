...
///
/// \file
/// Syscall wrapper for timer_create.
///
//===----------------------------------------------------------------------===//
...
timer_create(clockid_t clock_id, const struct sigevent *evp, int *timerid) {
  return syscall_checked<int>(SYS_timer_create, clock_id, evp, timerid);
}
