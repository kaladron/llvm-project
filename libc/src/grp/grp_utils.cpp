...
// Parses the colon-separated fields (name:passwd:gid:members) common to group
// database lines. Validates structure, numeric GID, and absence of trailing
// delimiters. Returns false on format violation.
...
//
// Like glibc and musl, a single static buffer and struct group are reused
// across getgrent, getgrnam, and getgrgid per POSIX ("The return value may
// point to a static area which is overwritten by a subsequent call to
// getgrent(), getgrgid(), or getgrnam()"), growing only to the high-water mark
...
ErrorOr<struct group *> find_by_name(cpp::string_view name) {
  auto res = lookup_by_name(name, &grp_entry, line_buffer, group_file_path);
  if (!res.has_value())
    return Error(res.error());
  if (!res.value())
    return nullptr;
...
ErrorOr<struct group *> find_by_gid(gid_t gid) {
  auto res = lookup_by_gid(gid, &grp_entry, line_buffer, group_file_path);
  if (!res.has_value())
    return Error(res.error());
  if (!res.value())
    return nullptr;
