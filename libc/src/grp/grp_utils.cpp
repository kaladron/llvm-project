...
//
// Like glibc and musl, a single static buffer is reused across non-reentrant
// group calls via realloc, growing only to the high-water mark of the largest
// record seen. endgrent() closes the file stream without freeing the buffer so
// that pointers returned prior to endgrent() remain valid until the next
// non-reentrant call.
