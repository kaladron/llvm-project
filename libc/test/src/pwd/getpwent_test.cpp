...
TEST_F(LlvmLibcPwdTest, EndPwentClosesStreamAndIterationRestarts) {
  ...
  // Like glibc and musl, endpwent closes the file stream without freeing the
  // static buffer, so the last returned pointer remains valid and the next
  // iteration reopens from the top.
  LIBC_NAMESPACE::endpwent();
  ASSERT_STREQ(pwd->pw_name, "root");
