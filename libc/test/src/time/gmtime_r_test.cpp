...
TEST_F(LlvmLibcGmTimeR, NullPtr) {
  struct tm tm_data;
  time_t seconds = 0;
  EXPECT_DEATH([] { LIBC_NAMESPACE::gmtime_r(nullptr, nullptr); }, WITH_SIGNAL(4));
  EXPECT_DEATH([&] { LIBC_NAMESPACE::gmtime_r(nullptr, &tm_data); }, WITH_SIGNAL(4));
  EXPECT_DEATH([&] { LIBC_NAMESPACE::gmtime_r(&seconds, nullptr); }, WITH_SIGNAL(4));
}
