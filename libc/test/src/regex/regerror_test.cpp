...
TEST(LlvmLibcRegexTest, RegerrorBasicCodes) {
  ...
  ASSERT_GT(regerror(REG_NOMATCH, nullptr, buf, sizeof(buf)),
            size_t(0));
  ASSERT_STREQ("No match", buf);

  ASSERT_GT(regerror(REG_ESPACE, nullptr, buf, sizeof(buf)),
            size_t(0));
...
TEST(LlvmLibcRegexTest, RegerrorTruncation) {
  char buf[5];
  size_t needed =
      regerror(REG_NOMATCH, nullptr, buf, sizeof(buf));
...
TEST(LlvmLibcRegexTest, RegerrorZeroBuffer) {
  size_t needed = regerror(REG_NOMATCH, nullptr, nullptr, 0);
  ASSERT_GT(needed, size_t(0));
}

TEST(LlvmLibcRegexTest, RegerrorSuccess) {
  char buf[128];
  regerror(0, nullptr, buf, sizeof(buf));
